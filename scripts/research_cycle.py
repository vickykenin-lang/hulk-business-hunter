...
SERPER_PROBE_QUERY='India business opportunities market trends'
def load(p,d):
    try:return json.loads(p.read_text(encoding='utf-8'))
    except Exception:return d

def serper_health_check(api_key, opener=urlopen):
    """Run a small authenticated search without returning or logging the key."""
    if not api_key:
        return {'health':'CREDENTIAL_MISSING','result_count':0}
    body=json.dumps({'q':SERPER_PROBE_QUERY,'num':3}).encode('utf-8')
    request=Request(SERPER_ENDPOINT,data=body,method='POST',headers={
        'X-API-KEY':api_key,
...
def main():
    ...
    search=serper_health_check(os.getenv('SERPER_API_KEY_HULK'))
    ...
    result={'checked_at_utc':datetime.now(timezone.utc).isoformat(timespec='seconds'),'state':state,'soul_valid':ok,'business_execution_authorized':False,'memory_status':mem.get('status'),'search_provider':'SERPER','search_health':search,'credential_values_stored':False}
